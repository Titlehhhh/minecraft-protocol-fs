...
(e.g. `tools\mcproto-facts.cmd packet play.toClient.keep_alive --format toon`); the full
CLI command, MCP tool, and REST endpoint lists live in `AI_CONTEXT.md`.

Raw `minecraft-data`: canon — the workspace AGENTS.md (forbidden always; a surface gap =
stop and ask the owner). The exceptions below exist for human developers of the loader
only: open raw `minecraft-data` files only when investigating a loader/parser bug,
validating an upstream-data discrepancy, or changing the loader itself. For Claude these
cases too are closed by the deny rules in `.claude\settings.json` — do not lift those
rules.

## Public Surfaces

The surfaces (protocol access layer, CLI, stdio MCP, Web UI / REST / HTTP MCP) and their
full command and endpoint lists live in `AI_CONTEXT.md`; per-project boundaries live in
`src/*/AGENTS.md`.
