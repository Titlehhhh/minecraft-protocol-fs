# McProtoFacts AI Context

McProtoFacts turns PrismarineJS `minecraft-data` protocol definitions into a queryable
protocol data model, then exposes that model through CLI, MCP, REST, Web UI, and graph
surfaces. It is a protocol facts provider: it does not generate code itself — consumers of
the facts do.
...
ProtoDef JSON documents. Agents do not inspect them directly — raw `minecraft-data`:
canon — the workspace AGENTS.md (forbidden always; a surface gap = stop and ask the
owner).
...
`McProtoFacts.Protocol` builds the usable protocol access layer:
...
tools\mcproto-facts.cmd packets --filter "player|move" --format json
...
For browser, REST, and HTTP MCP workflows, run `src/McpServer`.
...
`McpServer` exposes read-only protocol endpoints, a Web UI, and HTTP MCP transport. The
endpoints are useful when another process is already running the server:
...
- The protocol access layer is the shared source for CLI, MCP, REST, and graph surfaces.
- Code generation is out of scope. McProtoFacts provides protocol facts; code is
  generated by consumers of those facts.
- CLI and MCP wrappers should remain thin.
- Raw `minecraft-data` is not used as packet examples — canon: the workspace AGENTS.md;
  the only exceptions (loader/parser work by human developers) live in this folder's
  `AGENTS.md`, and for Claude they are closed by the deny rules in `.claude\settings.json`.
