...
module Helpers =
    ...
    let rec apiOf (w: WireType) : ApiType =
        match w with
        | Bool -> TBool
        | VarInt | I8 | U8 | I16 | U16 | I32 | U32 -> TInt
        | VarLong | I64 | U64 -> TLong
        | F32 -> TFloat
        | F64 -> TDouble
        | Str -> TString
        | Uuid -> TUuid
        | Nbt | AnonNbt -> TNbt
        | ByteArray | FixedBytes _ -> TBytes
    ...
    type Column = { Wire: string; Type: WireType; Api: string }

    let private pascal (s: string) =
        if s.Length = 0 then s else string (System.Char.ToUpper s.[0]) + s.[1..]

    /// A wire field whose api name is Pascal(wire) and api type is derived from the wire type.
    let col wire t = { Wire = wire; Type = t; Api = pascal wire }
    ...
    let record name range (cols: Column list) : NamedTypeSpec =
        { Name      = name
          ApiFields = cols |> List.map (fun c -> { Name = c.Api; Type = apiOf c.Type; Present = range })
          Layouts   = [ { Range = range; Entries = cols |> List.map (fun c -> Read(c.Wire, c.Type, c.Api)) } ] }
