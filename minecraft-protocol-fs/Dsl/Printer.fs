...
module Printer =
    ...
    let rec printEntry indent (entry: WireEntry) =
        ...
        match entry with
        | Read(wire, t, api) -> printfn "%sread %-20s -> %-20s [%A]" pad wire api t

        | Discard(wire, t) -> printfn "%sdiscard %-17s [%A]" pad wire t
        ...
        | ReadUnion(disc, unionName, api) -> printfn "%sreadUnion %s -> %s : %s" pad disc api unionName
